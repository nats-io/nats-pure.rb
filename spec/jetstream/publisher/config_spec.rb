...
RSpec.describe NATS::JetStream::Publisher::Config do
  ...
  describe "#headers" do
    ...
    context "with :stream" do
      ...
      it "includes Nats-Expected-Stream" do
        expect(subject.headers).to eq({"Nats-Expected-Stream" => "stream"})
      end
    end
    ...
    context "with :message_id" do
      ...
      it "includes Nats-Msg-Id" do
        expect(subject.headers).to eq({"Nats-Msg-Id" => "9f01"})
      end
    end
    ...
    context "with :last_message_id" do
      ...
      it "includes Nats-Expected-Last-Msg-Id" do
        expect(subject.headers).to eq({"Nats-Expected-Last-Msg-Id" => "8c34"})
      end
    end
    ...
    context "with :last_seq" do
      ...
      it "includes Nats-Expected-Last-Sequence" do
        expect(subject.headers).to eq({"Nats-Expected-Last-Sequence" => "5"})
      end
    end
    ...
    context "with :last_subject_seq" do
      ...
      it "includes Nats-Expected-Last-Subject-Sequence" do
        expect(subject.headers).to eq({"Nats-Expected-Last-Subject-Sequence" => "7"})
      end
    end
    ...
    context "with :rollup" do
      ...
      it "includes Nats-Rollup" do
        expect(subject.headers).to eq({"Nats-Rollup" => "stream"})
      end
    end
    ...
    context "with :header" do
      ...
      it "includes custom headers" do
        expect(subject.headers).to eq({"Header" => "Value"})
      end
    end
  end
end
