...
require_relative "../pull/config_examples"
...
RSpec.describe NATS::JetStream::Fetch::Config do
  ...
  context "#idle_heartbeats" do
    context "when idle_heartbeats is not specified" do
      context "and expires > 10 seconds" do
        let(:values) { {expires: 15.to_nsec} }

        it "sets idle_heartbeats to 5 seconds" do
          expect(subject.idle_heartbeat).to eq(5.to_nsec)
        end
      end

      context "and expires = 10 seconds" do
        let(:values) { {expires: 10.to_nsec} }
